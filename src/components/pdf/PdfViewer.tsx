...
const PageShell = React.memo(({ pageNumber, renderedWidth, renderedHeight, scale, renderPage, register }: PageShellProps) => (
  <div
    ref={(element) => register(pageNumber, element)}
    data-page={pageNumber}
    className="flex justify-center scroll-mt-4"
    style={{
      width: 'calc(var(--pdf-visual-zoom, 1) * ' + renderedWidth + 'px)',
      minWidth: 'calc(var(--pdf-visual-zoom, 1) * ' + renderedWidth + 'px)',
      height: 'calc(var(--pdf-visual-zoom, 1) * ' + renderedHeight + 'px)',
    }}
  >
    <div
      ...
      style={{
        ...
        transform: 'scale(var(--pdf-visual-zoom, 1))',
      }}
    >
      {renderPage ? (
        <Page
...
export const PdfViewer: React.FC<PdfViewerProps> = ({ resource, subject, folder, folderHierarchy = [] }) => {
  ...
  const pinchZoomRef = useRef(userZoom);
  ...
  useEffect(() => {
    ...
    if (isCompactViewport && autoFitRef.current) {
      ...
      setUserZoom((zoom) => Math.max(nextMinZoom, Math.min(MAX_ZOOM, zoom)));
    }
  }, [availableWidth, originalPageWidth]);

  const handleZoomChange = (newZoom: number) => {
    autoFitRef.current = false;
    const nextZoom = Math.min(MAX_ZOOM, Math.max(minZoom, newZoom));
    userZoomRef.current = nextZoom;
    viewportRef.current?.style.setProperty('--pdf-visual-zoom', String(nextZoom / renderedZoom));
    setUserZoom(nextZoom);
  };

  useEffect(() => {
    if (!fitScale || !originalPageWidth) return;
    ...
    pinchRenderTimerRef.current = window.setTimeout(() => {
      setRenderedZoom(userZoomRef.current);
      pinchRenderTimerRef.current = null;
    }, 160);
    return () => {
      if (pinchRenderTimerRef.current !== null) window.clearTimeout(pinchRenderTimerRef.current);
    };
  }, [fitScale, userZoom, originalPageWidth]);

  useEffect(() => {
    if (renderedZoom === userZoom) return;
    const timer = window.setTimeout(() => {
      viewportRef.current?.style.setProperty('--pdf-visual-zoom', '1');
    }, 300);
    return () => window.clearTimeout(timer);
  }, [renderedZoom, userZoom]);

  useEffect(() => () => {
    if (visualZoomFrameRef.current !== null) cancelAnimationFrame(visualZoomFrameRef.current);
    if (pinchRenderTimerRef.current !== null) window.clearTimeout(pinchRenderTimerRef.current);
  }, []);
  ...
  const handlePointerMove = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    const viewport = event.currentTarget;
    autoFitRef.current = false;
    if (visualZoomFrameRef.current !== null) return;
    visualZoomFrameRef.current = requestAnimationFrame(() => {
      ...
      const visualZoom = nextZoom / renderedZoom;
      pinchZoomRef.current = nextZoom;
      viewport.style.setProperty('--pdf-visual-zoom', String(visualZoom));
      viewport.scrollLeft = focalX * visualZoom - (pinchStartRef.current.midpointX - rect.left);
      viewport.scrollTop = focalY * visualZoom - (pinchStartRef.current.midpointY - rect.top);
    });
  };
  ...
  const handlePointerEnd = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (touchPointsRef.current.size < 2) {
      if (pinchStartRef.current) setUserZoom(pinchZoomRef.current);
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      if ((event.ctrlKey || event.metaKey) && (event.key === '+' || event.key === '=')) {
        event.preventDefault();
        handleZoomChange(userZoomRef.current + 0.15);
  ...
  return (
    <div ref={viewerRootRef} id="academic-pdf-viewer-root" className="flex flex-col h-[calc(100vh-60px)] sm:h-[calc(100vh-64px)] bg-slate-100 overflow-hidden">
        <PdfHeader resource={resource} zoomLevel={userZoom} minZoom={minZoom} maxZoom={MAX_ZOOM} onZoomChange={handleZoomChange} isFullscreen={isFullscreen} onToggleFullscreen={toggleFullscreen} isInfoOpen={isInfoOpen} onToggleInfo={() => setIsInfoOpen((open) => !open)} onToggleSidebar={() => setIsSidebarOpen((open) => !open)} />
      <Document
        ...
        onLoadSuccess={async (pdf) => {
          ...
          setOriginalPageWidth(baseViewport.width);
          setOriginalPageHeight(baseViewport.height);
          setPdfError(false);
        }}
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        <div ref={viewportRef} id="pdf-scroll-viewport" className="flex-1 min-w-0 w-full max-w-full overflow-auto overscroll-contain bg-slate-200/70 p-4 sm:p-8" style={{ touchAction: 'none' }} onPointerDown={handlePointerDown} onPointerMove={handlePointerMove} onPointerUp={handlePointerEnd} onPointerCancel={handlePointerEnd}>
          {!pdfError && originalPageWidth > 0 && originalPageHeight > 0 && pageNumbers(pageCount).map((page) => {
            const renderScale = fitScale * renderedZoom;
            const renderedWidth = Math.floor(originalPageWidth * renderScale);
            const renderedHeight = Math.floor(originalPageHeight * renderScale);
            return <PageShell key={page} pageNumber={page} renderedWidth={renderedWidth} renderedHeight={renderedHeight} scale={renderScale} renderPage={renderedPages.has(page)} register={registerPage} />;
          })}
        </div>
      ...
      <footer id="pdf-page-toolbar" className="bg-white/95 backdrop-blur-md border-t border-slate-200/90 px-4 py-2 flex items-center justify-between gap-4 z-20 shrink-0">
        ...
        <div className="text-xs text-slate-400 font-mono hidden sm:block">Zoom: {Math.round(userZoom * 100)}%</div>
      </footer>
    </div>
  );
};
